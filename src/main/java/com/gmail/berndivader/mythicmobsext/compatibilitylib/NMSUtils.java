 ...
 * 
 * This is not meant to be a replacement for full-on NMS or Protocol libs, but
 * it is enough for Magic to use internally without requiring any external
 * dependencies.
 * 
...
public class NMSUtils {
	...
	public static boolean initialize() {
		...
		try {
			...
			class_PacketPlayOutSpawnEntityLiving = fixBukkitClass(
			...
			class_PacketPlayOutBlockBreakAnimation = fixBukkitClass(
			...
			class_NBTTagCompound_setBooleanMethod = class_NBTTagCompound.getMethod("setBoolean", String.class,
					Boolean.TYPE);
			class_NBTTagCompound_setStringMethod = class_NBTTagCompound.getMethod("setString", String.class,
					String.class);
			class_NBTTagCompound_setDoubleMethod = class_NBTTagCompound.getMethod("setDouble", String.class,
					Double.TYPE);
			...
			class_CraftItemStack_copyMethod = class_CraftItemStack.getMethod("asNMSCopy",
			...
			class_World_addEntityMethod = class_World.getMethod("addEntity", class_Entity,
					CreatureSpawnEvent.SpawnReason.class);
			class_Entity_setLocationMethod = class_Entity.getMethod("setLocation", Double.TYPE, Double.TYPE,
			...
			class_EntityPlayer_setResourcePackMethod = class_EntityPlayer.getMethod("setResourcePack", String.class,
					String.class);
			...
			class_CraftInventoryCustom_constructor = class_CraftInventoryCustom.getConstructor(InventoryHolder.class,
					Integer.TYPE, String.class);
			class_EntityFireworkConstructor = class_EntityFirework.getConstructor(class_World, Double.TYPE, Double.TYPE,
					Double.TYPE, class_ItemStack);
			class_PacketSpawnEntityConstructor = class_PacketPlayOutSpawnEntity.getConstructor(class_Entity,
					Integer.TYPE);
			class_PacketSpawnLivingEntityConstructor = class_PacketPlayOutSpawnEntityLiving
					.getConstructor(class_EntityLiving);
			class_PacketPlayOutEntityMetadata_Constructor = class_PacketPlayOutEntityMetadata
					.getConstructor(Integer.TYPE, class_DataWatcher, Boolean.TYPE);
			class_PacketPlayOutEntityStatus_Constructor = class_PacketPlayOutEntityStatus.getConstructor(class_Entity,
					Byte.TYPE);
			class_PacketPlayOutEntityDestroy_Constructor = class_PacketPlayOutEntityDestroy.getConstructor(int[].class);
			class_PacketPlayOutExperience_Constructor = class_PacketPlayOutExperience.getConstructor(Float.TYPE,
					Integer.TYPE, Integer.TYPE);
			class_PacketPlayOutAnimation_Constructor = class_PacketPlayOutAnimation.getConstructor(class_Entity,
					Integer.TYPE);
			class_PacketPlayOutBlockBreakAnimation_Constructor = class_PacketPlayOutBlockBreakAnimation
					.getConstructor(Integer.TYPE, class_BlockPosition, Integer.TYPE);
			...
			class_GameProfileProperty_constructor = class_GameProfileProperty.getConstructor(String.class, String.class,
					String.class);
			...
			class_EntityPaintingConstructor = class_EntityPainting.getConstructor(class_World, class_BlockPosition,
					class_EnumDirection);
			class_EntityItemFrameConstructor = class_EntityItemFrame.getConstructor(class_World, class_BlockPosition,
					class_EnumDirection);
			...
			class_AxisAlignedBB_Constructor = class_AxisAlignedBB.getConstructor(Double.TYPE, Double.TYPE, Double.TYPE,
			...
			// We don't want to consider new-ish builds as "legacy" and print a warning, so
			// keep a separate flag
			...
			try {
				...
				class_World_explodeMethod = class_World.getMethod("createExplosion", class_Entity, Double.TYPE,
						Double.TYPE, Double.TYPE, Float.TYPE, Boolean.TYPE, class_EnumExplosionEffect);
				...
				try {
					class_World_explodeMethod = class_World.getMethod("createExplosion", class_Entity, Double.TYPE,
							Double.TYPE, Double.TYPE, Float.TYPE, Boolean.TYPE, Boolean.TYPE);
				} catch (Throwable ex) {
					Bukkit.getLogger().warning(
							"Could not bind to createExplosion method, magic explosions will not be attributed to the caster");
				...
				try {
					...
					for (Field field : class_entityTypes.getFields()) {
						if (field.getType().equals(class_entityTypes)) {
							...
							// This won't really work for all entity types, but it should work for most
							// projectiles
			...
			try {
				class_PacketPlayOutCustomSoundEffect = fixBukkitClass(
						"net.minecraft.server.PacketPlayOutCustomSoundEffect");

				if (class_MinecraftKey_constructor != null) {
					class_PacketPlayOutCustomSoundEffect_Constructor = class_PacketPlayOutCustomSoundEffect
							.getConstructor(class_MinecraftKey, class_EnumSoundCategory, class_Vec3D, Float.TYPE,
									Float.TYPE);
				} else {
					class_PacketPlayOutCustomSoundEffect_Constructor = class_PacketPlayOutCustomSoundEffect
							.getConstructor(String.class, class_EnumSoundCategory, Double.TYPE, Double.TYPE,
									Double.TYPE, Float.TYPE, Float.TYPE);
				}
			} catch (Throwable ex) {
				Bukkit.getLogger()
						.warning("Could not bind to custom effect method, custom sound effects will not work");
			}

			try {
				Class<?> class_ItemBoneMeal = fixBukkitClass("net.minecraft.server.ItemBoneMeal");
				class_ItemDye_bonemealMethod = class_ItemBoneMeal.getMethod("a", class_ItemStack, class_World,
						class_BlockPosition);
			} catch (Throwable not13) {
				try {
					Class<?> class_ItemDye = fixBukkitClass("net.minecraft.server.ItemDye");
					class_ItemDye_bonemealMethod = class_ItemDye.getMethod("a", class_ItemStack, class_World,
							class_BlockPosition);
				} catch (Throwable ex) {
			...
			try {
				...
				Bukkit.getLogger().log(Level.WARNING,
						"An error occurred while registering NBTTagList.getDouble, loading entities from schematics will not work",
						ex);
			...
			try {
				...
				class_World_setTypeAndDataMethod = class_World.getMethod("setTypeAndData", class_BlockPosition,
						class_IBlockData, Integer.TYPE);
			} catch (Throwable ex) {
				Bukkit.getLogger().log(Level.WARNING,
						"An error occurred while registering World.setTypeAndData, Deferred physics updates will not work",
						ex);
			...
			try {
				try {
					class_DamageSource_getMagicSourceMethod = class_DamageSource.getMethod("c", class_Entity,
							class_Entity);
				} catch (Throwable not13) {
					class_DamageSource_getMagicSourceMethod = class_DamageSource.getMethod("b", class_Entity,
							class_Entity);
				}
				class_EntityLiving_damageEntityMethod = class_EntityLiving.getMethod("damageEntity", class_DamageSource,
						Float.TYPE);
				Field damageSource_MagicField = class_DamageSource.getField("MAGIC");
				object_magicSource = damageSource_MagicField.get(null);
			} catch (Throwable ex) {
				Bukkit.getLogger().log(Level.WARNING,
						"An error occurred, magic damage will not work, using normal damage instead", ex);
			...
			try {
				...
				Bukkit.getLogger()
						.info("Could not bind to PotionMeta color methods, Custom colored potions will not work.");
			}
			try {
				class_Block_setTypeIdAndDataMethod = Block.class.getMethod("setTypeIdAndData", Integer.TYPE, Byte.TYPE,
						Boolean.TYPE);
			} catch (Throwable ex) {
				if (!isModernVersion) {
					Bukkit.getLogger()
							.info("Could not bind to setTypeIdAndData, Magic will have issues modifying blocks");
				}
			}
			try {
				Class<?> class_IBlockData = fixBukkitClass("net.minecraft.server.IBlockData");
				...
				if (!isModernVersion) {
					Bukkit.getLogger().log(Level.WARNING,
							"An error occurred while registering Block.fromLegacyData, setting fast blocks will not work.");
				}
			}
			try {
			...
			try {
				...
				if (!isCurrentVersion) {
					Bukkit.getLogger()
							.info("Couldn't bind to Chunk.isReady, building in ungenerated chunks may be glitchy.");
				}
			}
			...
			try {
				...
				class_ShapedRecipe_constructor = ShapedRecipe.class.getConstructor(class_NamespacedKey,
						ItemStack.class);
			} catch (Throwable ex) {
				...
				Bukkit.getLogger().info(
						"Couldn't find NamespacedKey for registering recipes. This is normal for legacy Minecraft versions.");
			}
			...
			if (class_NamespacedKey != null) {
				try {
					...
					Bukkit.getLogger().info(
							"Couldn't register knowledge book methods, recipe books unavailable. This is normal for pre-1.13 Minecraft versions.");
				}
			}
			...
			try {
				...
				Bukkit.getLogger()
						.info("Could not register Server.getEntity, entity lookups will be slightly less optimal");
			}
			try {
				...
				Bukkit.getLogger()
						.info("Could not register ProjectileHitEvent.getHitBlock, arrow hit locations will be fuzzy");
			}
			try {
				...
				try {
					...
					Bukkit.getLogger()
							.info("Could not register Arrow.PickupStatus, arrows can not be made to be picked up");
				}
			}
			try {
				...
				Bukkit.getLogger().log(Level.WARNING,
						"An error occurred while registering Player.getProfile, player portrait maps may not work as well",
						ex);
			}
			...
			try {
				...
				Bukkit.getLogger().log(Level.WARNING,
						"An error occurred while registering entity movement accessors, vehicle control will not work",
						ex);
			...
			try {
				try {
					...
					class_CraftMagicNumbers_getBlockMethod = craftMagicNumbers.getMethod("getBlock", Material.class);
						...
						Class<?> class_BlockBase = fixBukkitClass("net.minecraft.server.BlockBase");
				...
				Bukkit.getLogger().log(Level.WARNING,
						"An error occurred while registering block durability accessor, durability-based block checks will not work",
						ex);
			...
			try {
				...
				try {
					...
					class_PacketPlayOutChat_constructor = class_PacketPlayOutChat
							.getConstructor(class_IChatBaseComponent, class_ChatMessageType);
					...
					current = false;
					class_PacketPlayOutChat_constructor = class_PacketPlayOutChat
							.getConstructor(class_IChatBaseComponent, class_ChatMessageType, UUID.class);
					} catch (Throwable ex2) {
						// 1.11 fallback
						current = false;
						class_PacketPlayOutChat_constructor = class_PacketPlayOutChat
				...
				Bukkit.getLogger().log(Level.WARNING,
						"An error occurred while registering action bar methods, action bar messages will not work",
						ex);
			...
			try {
				try {
					// 1.11
					class_CraftWorld_createEntityMethod = class_CraftWorld.getMethod("createEntity", Location.class,
							Class.class);
					class_Consumer = fixBukkitClass("org.bukkit.util.Consumer");
					class_CraftWorld_spawnMethod = class_CraftWorld.getMethod("spawn", Location.class, Class.class,
							class_Consumer, CreatureSpawnEvent.SpawnReason.class);
				...
				Bukkit.getLogger().log(Level.WARNING,
						"An error occurred while registering custom spawn method, spawn reasons will not work", ex);
			...
			try {
				...
				try {
					class_CraftWorld_getTileEntityAtMethod = class_CraftWorld.getMethod("getTileEntityAt", Integer.TYPE,
							Integer.TYPE, Integer.TYPE);
				} catch (Throwable ignore) {
					// This should actually work in 1.10 and up at least, and is the preferred
					// method to use.
			...
			try {
				class_NBTCompressedStreamTools_loadFileMethod = class_NBTCompressedStreamTools.getMethod("a",
						InputStream.class);
			} catch (Throwable ex) {
			...
			try {
				try {
					...
					try {
						...
						class_TileEntityContainer_setLock = class_TileEntityContainer.getMethod("setLock",
								class_ChestLock);
					} catch (Throwable not12) {
						try {
							// 1.11
							class_ChestLock_getString = class_ChestLock.getMethod("b");
							class_TileEntityContainer_setLock = class_TileEntityContainer.getMethod("a",
									class_ChestLock);
						} catch (Throwable ignore) {
							// 1.10 and earlier
							setLegacy();
			...
			try {
				...
				try {
					...
					if (!class_EntityDamageSource_setThornsMethod.getReturnType()
							.isAssignableFrom(class_EntityDamageSource)) {
				...
				Bukkit.getLogger().log(Level.WARNING,
						"An error occurred, thorn damage override to hurt ender dragon will not work", ex);
			...
			try {
				...
				Bukkit.getLogger().log(Level.WARNING,
						"An error occurred, restoring inventories from schematics will not work", ex);
	...
	protected static void sendPacket(Server server, Location source, Collection<? extends Player> players,
	...
	public static boolean createExplosion(Entity entity, World world, double x, double y, double z, float power,
		...
		try {
			...
			Object explosion = class_EnumExplosionEffect != null
					? class_World_explodeMethod.invoke(worldHandle, entityHandle, x, y, z, power, setFire,
							breakBlocks ? enum_ExplosionEffect_BREAK : enum_ExplosionEffect_NONE)
	...
	public static Object getTileEntityData(Location location) {
		if (class_TileEntity_saveMethod == null)
			return null;
	...
	public static Object getTileEntity(Location location) {
		if (class_World_getTileEntityMethod != null) {
			...
			try {
				...
				Object blockLocation = class_BlockPosition_Constructor.newInstance(location.getX(), location.getY(),
						location.getZ());
		...
		try {
			World world = location.getWorld();
			tileEntity = class_CraftWorld_getTileEntityAtMethod.invoke(world, location.getBlockX(),
	...
	public static void clearItems(Location location) {
		if (class_TileEntity_loadMethod == null || class_TileEntity_updateMethod == null
				|| class_TileEntity_saveMethod == null)
			return;
		if (location == null)
			return;
	...
	public static void playCustomSound(Player player, Location location, String sound, float volume, float pitch) {
		if (class_PacketPlayOutCustomSoundEffect_Constructor == null)
			return;
		try {
			Object packet = null;
			if (class_MinecraftKey_constructor != null) {
				...
				packet = class_PacketPlayOutCustomSoundEffect_Constructor.newInstance(key, enum_SoundCategory_PLAYERS,
						vec, volume, pitch);
			} else {
				packet = class_PacketPlayOutCustomSoundEffect_Constructor.newInstance(sound, enum_SoundCategory_PLAYERS,
	...
	public static boolean setMap(ConfigurationSection section, Map<String, Object> map) {
		if (section == null || class_MemorySection_mapField == null)
			return false;
	...
	protected static void addToList(Object listObject, Object node)
